...
Using Faraday's law:
\begin{align*}
    \bB_{\bk,\lambda}(z,t)=(\bk\times\be_{\lambda})\frac{1}{kc^2}\sqrt{\frac{2\omega^2}{V\varepsilon_0}}\dot{q}_{\bk,\lambda}(t)\cos(kz).
\end{align*}
...
\begin{align}
    \shortstack{Quantized single-mode EM fields}\qquad\highlight{\begin{array}{l}
...
\begin{align*}
    \ha^\dagger_{\bk,\lambda}\ket{0}=\ket{1}_{\bk,\lambda},\quad\ha_{\bk,\lambda}\ket{n}_{\bk,\lambda}=\sqrt{n}\ket{n-1}_{\bk,\lambda},
...
\begin{align}
    \text{Quantized multi-mode EM fields}\quad\highlight{\begin{array}{l}
        \displaystyle\bE(z,t)=\sum_{m,\lambda}\he_\lambda\sqrt{\frac{2\omega_m^2}{\varepsilon_0V}}\hq_{m,\lambda}(t)\sin(k_mz)\\
        \displaystyle\bB(z,t)=\sum_{m,\lambda}(\hz\times\he_\lambda)\frac{1}{k_mc^2}\sqrt{\frac{2\omega_m^2}{\varepsilon_0V}}\dot{\hq}_{m,\lambda}(t)\cos(k_mz)
    \end{array}}.
\end{align}
...
\begin{align*}
    ...
    \end{cases}.
\end{align*}
In Coulomb gauge, in absence of charge, we have $\phi=0$ such that the vector potential-wave equation is 
\begin{align*}
...
\begin{align*}
    ...
    \bB(\br,t)&=\nabla\times\bA(\br,t)=i\sum_{\bk,\lambda}(\bk\times\he_{\bk,\lambda})\left[A_{\bk,\lambda}e^{i(\bk\cdot\br-\omega t)}-A^*_{\bk,\lambda}e^{-i(\bk\cdot\br-\omega t)}\right].
\end{align*}
...
\begin{align*}
    ...
    &=\text{Square becomes a dot product of itself for each field}\\
    &\vdots\\
...
Lets conside one mode of the E-field in a number state: $\ket{n}_{\bk,\lambda}\ket{\{0\}}$.
...
\begin{align*}
    _{\bk_0,\lambda_0}\braket{n|\hbE(\br,t)|n}_{\bk_0,\lambda_0}&=i_{\bk_0,\lambda_0}\braket{n|\sum_{\bk,\lambda}\he_{\bk,\lambda}\sqrt{\frac{\hbar\omega}{2\varepsilon_0V}}\left[\ha_{\bk,\lambda}(t)e^{i\bk\cdot\br}-\ha^\dagger_{\bk,\lambda}(t)e^{-i\bk\cdot\br}\right]|n}_{\bk_0,\lambda_0}\\
...
\end{align*}
Thus, there is an uncertainty in the E-field due to the \emph{quantum fluctuations}, even in vacuum.