...
For a two-state system, there always exists a description in terms of the Pauli operators $\hsigma_1,\hsigma_2,\hsigma_3$
satisfying the commutation relations $[\hsigma_i,\hsigma_j]=2i\varepsilon_{ijk}\hsigma_k$.

In a basis where $\hsigma_3$ and $\hsigma^2=\hsigma_1^2+\hsigma_2^2+\hsigma_3^2$ are diagonal, these operators can be written in matric form:
\begin{align}
    ...
    \end{bmatrix}.
\end{align}
Any Hermitian $2\times2$ matrix can be expressed in terms of the Pauli matrices and the identity matriz $\hat{I}_2$, and of course, $\hrho$.
We can then write:
\begin{align}
    \hrho=\begin{bmatrix}
        \rho_{11}&\rho_{12}\\\rho_{21}&\rho_{22}
    \end{bmatrix}=\frac{1}{2}\begin{bmatrix}
        1-s_3&s_1+is_2\\s_1-is_2&1-s_3
    \end{bmatrix}=\frac{1}{2}(\hat{I}_2+\bs\cdot\sigma),
\end{align}
where $\bs=\{s_1,s_2,s_3\}$ is the \emph{Bloch vector}. The associated quantum state can be represented as:
\begin{align}
    \highlight{\ket{\psi}=\cos\frac{\theta}{2}e^{-i\phi/2}\ket{\uparrow}+\sin\frac{\theta}{2}e^{i\phi/2}\ket{\downarrow}}.
\end{align}
The density operator above has two eigenvalues:
\begin{align}
    \highlight{g_{\substack{1\\2}}=\frac{1}{2}\left[1\pm\sqrt{s_1^2+s_2^2+s_3^2}\right]=\frac{1}{2}[1\pm|\bs|]},
\end{align}
and its eigenvectors are determined by the two vectors $\pm\bu$ shown in the Bloch sphere. For pure states where $|\bs|=1$, $\pm\bu=\pm\bs$ and $\bs$ lies 
on the surface of the Bloch sphere. For mixed states where $|\bs|<1$, $\bs$ lies inside the Bloch sphere and $\pm\bu=\pm\frac{\bs}{|\bs|}$.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.3\columnwidth]{Appendix/blochsphere.png}
    \caption{Representation of the density matrix of a two-state ssystem in terms of the Bloch sphere and Bloch vector.}
\end{figure}
%
...
density operator over the states of all the other systems. With $\phi=\ket{\psi}\bra{\psi}$, the reduced density operator for particle 1 is 
\begin{align*}
...
\begin{align}
    \text{von Neumann eq 1}\qquad\partial_t\hrho=\frac{i}{\hbar}[\hrho,\hH],\quad\text{with}\quad i\hbar\partial_t\ket{\psi_i}=\hH\ket{\psi_i}.
\end{align}
